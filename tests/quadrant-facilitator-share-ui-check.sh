...
view_contract="$(agent-browser --session "$view_session" eval '(()=>{const text=document.querySelector("#shared-facilitator-view").innerText;return localStorage.length===1&&localStorage.getItem("sentinel")==="unchanged"&&/Which launch/.test(text)&&/A limited release/.test(text)&&/2 responses/.test(text)&&!/Alex|Blair/.test(text)&&!document.querySelector("[data-participant-position]")})()')"
