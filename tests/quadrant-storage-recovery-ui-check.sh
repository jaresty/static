...
recovered="$(agent-browser --session "$session" eval '(()=>{const status=document.querySelector("#storage-status");const saved=JSON.parse(localStorage.getItem("quadrant:workshop:v1"));return Boolean(status&&!status.hidden&&/cleaned up old saved work/i.test(status.innerText)&&/saved your current work/i.test(status.innerText)&&saved.candidateId!=="item-2")})()')"
