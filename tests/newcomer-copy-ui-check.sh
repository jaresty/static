...
for app in 2x2-facilitator pairwise-ranker; do
  ...
  result="$(agent-browser --session "$session" eval '(()=>{const intro=document.querySelector("[data-newcomer-intro]")?.innerText||"";const cards=Array.from(document.querySelectorAll(".mode-card")).map(card=>card.innerText);const offer=document.querySelector("[data-tour-offer]")?.innerText||"";const quadrant=location.pathname.includes("2x2-facilitator");const method=quadrant?/places each option on two axes/i:/comparing two items at a time/i;const benefit=quadrant?/see trade-offs/i:/clear priority order/i;const solo=quadrant?/Create a 2×2 yourself[\s\S]*(yourself|your grid)/i:/Work solo[\s\S]*(yourself|on your own)/i;const invite=quadrant?/Create a 2×2 for a group[\s\S]*invite/i:/Invite responses[\s\S]*(same|share|send)/i;const combine=quadrant?/Combine completed responses[\s\S]*(compare|agreement|disagreement)/i:/Combine responses[\s\S]*(compare|agreement|disagreement)/i;return {introVisible:Boolean(document.querySelector("[data-newcomer-intro]")?.getBoundingClientRect().height),method:method.test(intro),benefit:benefit.test(intro),solo:solo.test(cards[0]||""),invite:invite.test(cards[1]||""),combine:combine.test(cards[2]||""),practice:/Practice the complete workflow/i.test(offer)}})()')"
